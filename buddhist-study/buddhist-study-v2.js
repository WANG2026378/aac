(()=>{
  ...
  function showReward(number,isNew=true){
    ...
    $('rewardDialog').classList.toggle('compact',isNew);
    $('continuousNote').textContent=voiceOn
      ?'🎙️ 語音計數仍在持續，請安心接著念佛。'
      :'提醒不會鎖住計數；可繼續念佛或按＋1。';
    clearTimeout(rewardTimer);
    if($('rewardDialog').open)$('rewardDialog').close();
    $('rewardDialog').show();
    if(isNew)rewardTimer=setTimeout(closeReward,15000);
  }
  ...
  function countLoosePhrases(transcript){
    const normalized=transcript
      .replace(/[\s，。！？、,.!?]/g,'')
      .replace(/弥/g,'彌')
      .replace(/阿[米咪密蜜迷]/g,'阿彌')
      .replace(/彌多/g,'彌陀')
      .replace(/豆腐/g,'陀佛');
    return (normalized.match(/阿彌陀(?:佛|福)?|彌陀(?:佛|福)?|阿彌/g)||[]).length;
  }
  ...
  function startVoice(){
    ...
    recognition.onresult=event=>{
      ...
      for(let index=event.resultIndex;index<event.results.length;index+=1){
        ...
        const heard=countLoosePhrases(transcript);
        const previous=heardByResult.get(index)||0;
        const delta=Math.max(0,heard-previous);
        if(delta){add(delta);newlyCounted+=delta}
        heardByResult.set(index,Math.max(previous,heard));
        if(!result.isFinal)interim+=transcript;
      }
      if(newlyCounted)voiceStatus(`寬鬆辨識新增 ${newlyCounted} 聲；本輪 ${roundCount()}／108。`);
      else if(interim)voiceStatus(`正在寬鬆辨識：${interim.slice(-18)}`);
    };
